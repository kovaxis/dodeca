/*
 * Tiny4kOLED - Drivers for SSD1306 controlled dot matrix OLED/PLED 128x32
 * displays
 *
...
static void ssd1306_begin(void) { wireBeginFn(); }

static void ssd1306_send_start(void) { wireBeginTransmissionFn(); }

static bool ssd1306_send_byte(uint8_t byte) { return wireWriteFn(byte); }

static void ssd1306_send_stop(void) { wireEndTransmissionFn(); }
...
static void ssd1306_send_command3(uint8_t command1, uint8_t command2,
                                  uint8_t command3) {
    ssd1306_send_command_start();
    ssd1306_send_byte(command1);
    ssd1306_send_byte(command2);
...
static void ssd1306_send_command6(uint8_t command1, uint8_t command2,
                                  uint8_t command3, uint8_t command4,
                                  uint8_t command5, uint8_t command6) {
    ssd1306_send_command_start();
    ssd1306_send_byte(command1);
    ssd1306_send_byte(command2);
    ssd1306_send_byte(command3);
...
static void ssd1306_send_command7(uint8_t command1, uint8_t command2,
                                  uint8_t command3, uint8_t command4,
                                  uint8_t command5, uint8_t command6,
                                  uint8_t command7) {
    ssd1306_send_command_start();
...
SSD1306Device::SSD1306Device(void (*wireBeginFunc)(void),
...
void SSD1306Device::begin(uint8_t init_sequence_length,
...
void SSD1306Device::begin(uint8_t xOffset, uint8_t yOffset, uint8_t width,
                          uint8_t height, uint8_t init_sequence_length,
                          const uint8_t init_sequence[]) {
    oledOffsetX = xOffset;
...
void SSD1306Device::setPages(uint8_t pages) { oledPages = pages; }

void SSD1306Device::setWidth(uint8_t width) { oledWidth = width; }

void SSD1306Device::setHeight(uint8_t height) { oledPages = height >> 3; }
...
void SSD1306Device::setFont(const DCfont *font) { oledFont = font; }

void SSD1306Device::setCursor(uint8_t x, uint8_t y) {
    ssd1306_send_command3(renderingFrame | ((y + oledOffsetY) & 0x07),
                          0x10 | (((x + oledOffsetX) & 0xf0) >> 4),
                          (x + oledOffsetX) & 0x0f);
    oledX = x;
    oledY = y;
}

void SSD1306Device::clear(void) { fill(0x00); }
...
void SSD1306Device::newLine(void) { newLine(oledFont->height); }

size_t SSD1306Device::write(byte c) {
    if (!oledFont) return 1;
    ...
    do {
        ...
        for (uint8_t i = 0; i < w; i++) {
            ssd1306_send_data_byte(
                pgm_read_byte(&(oledFont->bitmap[offset++])));
        }
        ssd1306_send_stop();
        if (h == 1) {
            oledX += w;
...
void SSD1306Device::bitmap(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,
...
void SSD1306Device::clearToEOL(void) { fillToEOL(0x00); }
...
void SSD1306Device::startData(void) { ssd1306_send_data_start(); }
...
void SSD1306Device::clearData(uint8_t length) { repeatData(0, length); }

void SSD1306Device::endData(void) { ssd1306_send_stop(); }

void SSD1306Device::invertOutput(bool enable) {
    invertedOutput = enable ? 0xff : 0;
}

void SSD1306Device::clipText(uint16_t startPixel, uint8_t width,
                             DATACUTE_F_MACRO_T *text) {
    uint8_t h = oledFont->height;
    uint8_t w = oledFont->width;
    PGM_P p = reinterpret_cast<PGM_P>(text);
    uint8_t drawnColumns = 0;
    // It is currently up to the caller to make sure that the startPixel is
    // still within the text. This method needs to change to read each character
    // in the string up to startPixel, to check for end of string If beyond the
    // end of the string, write 0s/spaces However, it probably takes less bytes
    // currently to simply put spaces at the beginning or end of the text.
    ...
    while (drawnColumns < width) {
        ...
        do {
            ...
            ssd1306_send_data_start();
            for (uint8_t i = 0;
                 (i < w - initialSkip) && ((drawnColumns + i) < width); i++) {
                ssd1306_send_data_byte(
                    pgm_read_byte(&(oledFont->bitmap[offset + i])));
            }
...
void SSD1306Device::switchRenderFrame(void) { renderingFrame ^= 0x04; }
...
void SSD1306Device::setExternalIref(void) { ssd1306_send_command2(0xAD, 0x00); }
...
void SSD1306Device::scrollRight(uint8_t startPage, uint8_t interval,
                                uint8_t endPage, uint8_t startColumn,
                                uint8_t endColumn) {
    ssd1306_send_command7(0x26, 0x00, startPage + oledOffsetY, interval,
                          endPage + oledOffsetY, startColumn, endColumn);
}

void SSD1306Device::scrollLeft(uint8_t startPage, uint8_t interval,
                               uint8_t endPage, uint8_t startColumn,
                               uint8_t endColumn) {
    ssd1306_send_command7(0x27, 0x00, startPage + oledOffsetY, interval,
                          endPage + oledOffsetY, startColumn, endColumn);
}

void SSD1306Device::scrollRightOffset(uint8_t startPage, uint8_t interval,
                                      uint8_t endPage, uint8_t offset) {
    ssd1306_send_command6(0x29, 0x00, startPage + oledOffsetY, interval,
                          endPage + oledOffsetY, offset);
}

void SSD1306Device::scrollLeftOffset(uint8_t startPage, uint8_t interval,
                                     uint8_t endPage, uint8_t offset) {
    ssd1306_send_command6(0x2A, 0x00, startPage + oledOffsetY, interval,
                          endPage + oledOffsetY, offset);
}

void SSD1306Device::scrollContentRight(uint8_t startPage, uint8_t endPage,
                                       uint8_t startColumn, uint8_t endColumn) {
    ssd1306_send_command7(0x2C, 0x00, startPage + oledOffsetY, 0x01,
                          endPage + oledOffsetY, startColumn + oledOffsetX,
                          endColumn + oledOffsetX);
}

void SSD1306Device::scrollContentLeft(uint8_t startPage, uint8_t endPage,
                                      uint8_t startColumn, uint8_t endColumn) {
    ssd1306_send_command7(0x2D, 0x00, startPage + oledOffsetY, 0x01,
                          endPage + oledOffsetY, startColumn + oledOffsetX,
                          endColumn + oledOffsetX);
}

void SSD1306Device::deactivateScroll(void) { ssd1306_send_command(0x2E); }

void SSD1306Device::activateScroll(void) { ssd1306_send_command(0x2F); }
...
void SSD1306Device::setComPinsHardwareConfiguration(
    uint8_t alternative, uint8_t enableLeftRightRemap) {
    ssd1306_send_command2(0xDA, ((enableLeftRightRemap & 0x01) << 5) |
                                    ((alternative & 0x01) << 4) | 0x02);
}

// 5. Timing and Driving Scheme Setting Command table

void SSD1306Device::setDisplayClock(uint8_t divideRatio,
                                    uint8_t oscillatorFrequency) {
    ssd1306_send_command2(
        0xD5, ((oscillatorFrequency & 0x0F) << 4) | ((divideRatio - 1) & 0x0F));
}

void SSD1306Device::setPrechargePeriod(uint8_t phaseOnePeriod,
                                       uint8_t phaseTwoPeriod) {
    ssd1306_send_command2(
        0xD9, ((phaseTwoPeriod & 0x0F) << 4) | (phaseOnePeriod & 0x0F));
}
...
void SSD1306Device::nop(void) { ssd1306_send_command(0xE3); }
...
void SSD1306Device::enableZoomIn(void) { ssd1306_send_command2(0xD6, 0x01); }

void SSD1306Device::disableZoomIn(void) { ssd1306_send_command2(0xD6, 0x00); }
