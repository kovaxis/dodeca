...
typedef struct {
    ...
    byte width;
    byte height;
    // 5 different positions, each an X/Y pair of pixel positions for the
    // top-left corner of a character
    byte positions[10];
    // Pointer to character data in PROGMEM
    // Each character is the same length and size, and consists of a series of
    // bits, in column first order, from top to bottom. There are 2*CHAR_COUNT
    // characters in here (twice the amount, because each character has a 90
    // degree rotated form).
    const byte *data;
} Font;
...
void draw_upright(int width, int height, int base_x, int base_y,
                  const byte *data) {
    // Write data
...
void draw_upsidedown(int width, int height, int base_x, int base_y,
                     const byte *data) {
    // Write data
    int min_page = base_y / 8;
    int page_offset = base_y % 8;
    ...
    for (int page = min_page; page <= max_page; page += 1) {
        ...
        for (int dx = 0; dx < width; dx += 1) {
            int offset = width * height + page_offset - (page - min_page) * 8 -
                         dx * height;
            int byte_offset = (offset + 8) / 8 - 1;
            int bit_offset = (offset + 8) % 8;
...
void scr_draw_bat_sprite(BatStatus bat_status) {
    ...
    draw_upright(sprite.width, sprite.height, sprite.default_x,
                 sprite.default_y, data);
}
...
#undef MAX_CAP
    ...
    if ((rotate & 0x2) == 0) {
        ...
        draw_upsidedown(width, height, SCRBUF_WIDTH - x - width,
                        SCRBUF_PAGES * 8 - y - height, data);
    }
}
